...
class SpectrumFigure():
    ...
    def plot(self):
        self.axs[0].plot(
            self.unscattered_spectrum.x,
            self.unscattered_spectrum.lineshape)
        self.axs[0].plot(
            self.scattered_spectrum.x,
            self.scattered_spectrum.lineshape)
        self.axs[0].plot(
            self.simulated_spectrum.x,
            self.simulated_spectrum.lineshape)
        ax0_text = (
        ...
        self.axs[1].plot(self.loss_function.x,
                         self.loss_function.lineshape)
...
spectrum_filename = "SiO2 O1s synthetic"

filename = os.path.join(*[directory,
                          "data",
                          spectrum_filename + ".vms"])
...
for i in [unscattered_index, scattered_index]:
    ...
    model.loaded_spectra += [MeasuredSpectrum(data['data']['x'],
                                              data['data']['y0'])]
...
filenumber = len([f for f in os.listdir(output_folder) if f.startswith(spectrum_filename)])
fig_filepath = os.path.join(output_folder,
                            f"{spectrum_filename}_{filenumber}.png")
